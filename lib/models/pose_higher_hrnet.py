...
from .block import MBConv, GhostModule, SEMBConv
...
class PoseHigherResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        # self.layer1 = self._make_mblayer(MBConv, 64, 256, 4) # MBConv
        self.layer1 = self._make_mblayer(SEMBConv, 64, 256, 4) # MBConv
    ...
    def _make_mblayer(self, block, in_feature, out_feature, num_blocks):
        layers = []
        layers.append(block(in_feature, 32, 1))
        for i in range(1, num_blocks-2):
            layers.append(block(32, 32, 2))
        layers.append(block(32, out_feature, 2))
    ...
    def _make_deconv_layers(self, cfg, input_channels):
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            ...
            for _ in range(cfg.MODEL.EXTRA.DECONV.NUM_BASIC_BLOCKS):
                ...
                # )
                layers.append(nn.Sequential(
                    GhostModule(output_channels, output_channels),
                )  # Ghost
                )
    ...
    def init_weights(self, pretrained='', verbose=True):
        ...
        best_ckpt = torch.load('output/coco_kpt/pose_higher_hrnet_mbconv+ghost/w32_512_adam_lr1e-3/model_best.pth.tar')
        self.load_state_dict(best_ckpt, strict=False)
